...
export default function ShortenerForm() {
    ...
    const [expirationRedirectUrl, setExpirationRedirectUrl] = useState("");
        ...
        try {
            const res = await fetch("/api/shorten", {
                ...
                body: JSON.stringify({
                    ...
                    expiration_redirect_url: expirationRedirectUrl || null
                }),
            });
    ...
    const handleAIAutoFill = async () => {
        ...
        try {
            ...
            const data = await res.json();
            if (res.ok) {
                if (data.title) setSocialTitle(data.title.substring(0, 60)); // Limit for SEO
                if (data.description) setSocialDescription(data.description.substring(0, 160));
                if (data.image) setSocialImage(data.image);
                        ...
                        {session && (
                            <div className="flex justify-end mb-4">
                                <button
                                    ...
                                    className="text-xs flex items-center gap-1.5 bg-purple-100 dark:bg-purple-900/30 text-purple-600 dark:text-purple-300 px-3 py-1.5 rounded-full hover:bg-purple-200 dark:hover:bg-purple-900/50 transition-colors disabled:opacity-50"
                                >
                                    {isAnalyzing ? (
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    ...
                                    <div className="relative group">
                                        ...
                                        <div className="relative flex items-center bg-white dark:bg-gray-900 rounded-xl shadow-md p-1.5 ring-1 ring-gray-900/5 dark:ring-white/10">
                                            ...
                                            <input
                                                type="url"
                                                placeholder="Custom Expired Redirect (optional)"
                                                value={expirationRedirectUrl}
                                                onChange={(e) => setExpirationRedirectUrl(e.target.value)}
                                                className="flex-1 w-full px-3 py-2 text-gray-900 dark:text-white placeholder-gray-400 dark:placeholder-gray-500 bg-transparent border-none focus:ring-0 focus:outline-none text-sm font-medium"
                                                title="Where to send users after link expires"
                                            />
                                        </div>
                                    </div>
                                </div>
